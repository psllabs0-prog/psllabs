import type { Product } from "@/lib/products";

import { AnimateIn } from "./animate-in";
import { ProductGallery } from "./product-gallery";
import { ProductPurchase } from "./product-purchase";
import { StarRating } from "./star-rating";

export function ProductHero({ product }: { product: Product }) {
  return (
    <section className="mx-auto max-w-[1440px] px-6 py-12 md:px-12 md:py-20 lg:px-24 lg:py-28">
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-20">
        <AnimateIn y={16}>
          <ProductGallery productName={product.name} />
        </AnimateIn>

        <div className="flex flex-col gap-8">
          <AnimateIn delay={0.08}>
            <p className="mono text-[var(--color-stone)]">{product.tag}</p>
          </AnimateIn>

          <AnimateIn delay={0.16}>
            <h1 className="font-[family-name:var(--font-display)] text-[clamp(2rem,4vw,3rem)] leading-[1.1] tracking-[-0.02em] text-[var(--color-ink)]">
              {product.name}
            </h1>
          </AnimateIn>

          <AnimateIn delay={0.24} className="flex flex-col gap-3">
            <StarRating />
            <p className="font-[family-name:var(--font-mono)] text-xs text-[var(--color-stone)]">
              {product.stackRole}
            </p>
          </AnimateIn>

          <AnimateIn delay={0.32}>
            <p className="max-w-md text-base leading-relaxed text-[var(--color-stone)] md:text-[1.0625rem]">
              {product.shortDescription}
            </p>
          </AnimateIn>

          <AnimateIn delay={0.4}>
            <ProductPurchase
              productHandle={product.handle}
              price={product.price}
              subscribePrice={product.subscribePrice}
              stockStatus={product.stockStatus}
            />
          </AnimateIn>

          <AnimateIn delay={0.48}>
            <ul className="flex flex-col gap-3 border-t border-[var(--color-sage)] pt-8">
              {product.bullets.map((bullet) => (
                <li
                  key={bullet}
                  className="flex gap-3 text-sm text-[var(--color-ink)]"
                >
                  <span
                    className="mt-0.5 font-[family-name:var(--font-mono)] text-xs text-[var(--color-sage)]"
                    aria-hidden
                  >
                    —
                  </span>
                  {bullet}
                </li>
              ))}
            </ul>
          </AnimateIn>
        </div>
      </div>
    </section>
  );
}
