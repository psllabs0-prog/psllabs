/**
 * Offline retention + support lease tests.
...
function testUtmsAndCopy() {
  ...
  const built = buildRetention30dEmail({ email: "a@b.com" });
  assert(/Current availability/i.test(built.subject), "subject");
  assert(!/running low|next dose|cycle|refill|treatment|results|benefits|effects/i.test(built.text), "banned words");
